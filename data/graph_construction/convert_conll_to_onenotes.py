...
def main(inp, domain, out):
    # df = pd.read_csv(inp, sep = "\t", skip_blank_lines = False) # header=None,
    df = pd.read_csv(inp, sep="\t", skip_blank_lines=False,
                     names=['word_id', 'word', 'pred', 'pred_id', 'head_pred_id', 'sent_id', 'run_id', 'label'])
    ...
    mask = df['word'].isnull()
    df = df.mask(mask)
    col_list = ['domain', 'part_number', 'word_number', 'word', 'pos', 'parse_bit', 'lemma', 'pred_frameset',
                'word_sense', 'speaker', 'named_entities', 'tags', 'coref', 'pred']
    df = df[col_list]
