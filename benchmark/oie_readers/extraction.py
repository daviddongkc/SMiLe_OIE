...
from benchmark.oie_readers.argument import Argument
...
class Extraction:
    ...
    def binarizeByIndex(self):
        ...
        sortedExtraction = sorted(markPred, key=lambda p: p[1][0])
    ...
    def getSortedArgs(self):
        ...
        return [a for a, _ in sorted(ls, key = lambda p: min(p[1]))]
    ...
    def sort_args_by_distribution(self):
        ...
        subj_question, subj_args = max(self.questions.iteritems(), key = lambda q: self.question_prob_for_loc(q[0], 0))
        ...
        #                                reverse = True):
        for (question, args) in sorted([(q, a) for (q, a) in self.questions.iteritems() if (q not in [subj_question])],
                                       key = lambda q: sum(self.question_dist[generalize_question(q[0])].values()), reverse = True):
            gen_question = generalize_question(question)
            arg = args[0]
            assigned_flag = False
            ...
            #                            key = lambda (_ , c): c,
            #                            reverse = True):
            for (loc, count) in sorted(self.question_dist[gen_question].iteritems(), key = lambda p: p[1], reverse = True):
                if loc not in ret:
        ...
        return [arg
                for (_, arg_ls) in sorted(ret.iteritems(), key = lambda p: int(p[0]))
                for arg in arg_ls]
    ...
    def get_label(self, index):
        ...
        elem_ind, elem = min(ent, key=lambda p: len(p[1]))
